...
import {
    closest,
    getOffsetRect,
    getTransformProperties as getTransformProperties,
    listWithChildren
} from './utils.js'
...
export default {
    ...
    computed: {
        ...
        listStyles () {
            const element = document.querySelector('.nestable-' + this.group + ' .nestable-item-' + this.dragItem[this.keyProp])

            let listStyles = {}

            if (element) {
                listStyles.width = `${element.clientWidth}px`
            }
    ...
    methods: {
        ...
        onMouseMove (event) {
            ...
            const transformProperties = getTransformProperties(clientX, clientY)
            ...
            const elementDragLayer = document.querySelector('.nestable-' + this.group + ' .nestable-drag-layer')
            if (!elementDragLayer) return
            const { top: dragLayerTop, left: dragLayerLeft } = elementDragLayer.getBoundingClientRect()

            const elementCopy = document.querySelector('.nestable-' + this.group + ' .nestable-drag-layer > .nestable-list')
                ...
                if (elementCopy) {
                    for (const key in transformProperties) {
                        if (Object.prototype.hasOwnProperty.call(transformProperties, key)) {
                            elementCopy.style[key] = transformProperties[key]
                        }
                    }
                }
        ...
        tryIncreaseDepth (dragItem) {
            ...
            if (itemIndex > 0 && newDepth <= this.maxDepth) {
                const previousSibling = this.getItemByPath(pathFrom.slice(0, -1).concat(itemIndex - 1))

                // previous sibling is not collapsed
                if (previousSibling[this.childrenProp] && (previousSibling[this.childrenProp].length === 0 || !this.isCollapsed(previousSibling))) {
                    const pathTo = pathFrom
                        .slice(0, -1)
                        .concat(itemIndex - 1)
                        .concat(previousSibling[this.childrenProp].length)
        ...
        onMouseEnter (event, eventList, item) {
            ...
            let collapseProperties = {}
            if (this.collapsed && pathFrom.length > 1) {
                const parent = this.getItemByPath(pathFrom.slice(0, -1))

                if (parent[this.childrenProp].length === 1) {
                    collapseProperties = this.onToggleCollapse(parent, true)
                }
            }

            this.moveItem({ dragItem, pathFrom, pathTo }, collapseProperties)
        },
